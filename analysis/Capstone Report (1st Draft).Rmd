...
```{r content_of_answer_1, tidy=TRUE, message=FALSE, warning=FALSE, results='hide', echo=FALSE, fig.align='center', results='asis'}
...
```{r composite_score_pie_chart, tidy=TRUE, message=FALSE, warning=FALSE, results='hide', echo=FALSE}
...
# Unique GROUP values
groups <- unique(lying$GROUP)

# Loop over each group
for (grp in groups) {
  
  # Filter and count buckets for this group
  bucket_counts_group <- lying %>%
    filter(GROUP == grp) %>%
    group_by(change_bucket) %>%
    summarise(count = n(), .groups = "drop") %>%
  ...
  p <- ggplot(bucket_counts_group, aes(x = "", y = count, fill = change_bucket)) +
    ...
    labs(
      title = paste("Composite Change Distribution – Group", grp),
      fill = "Change Bucket"
    ) +
    theme_void() +
    theme(
  ...
  # Optional: save the chart to file
  # ggsave(paste0("pie_chart_group_", grp, ".png"), plot = p, width = 6, height = 6)
}
...
## QQ3: Count of Change in Views/Answers (Use This One)
...
```{r ad_sentiment, qq3_stacked_bars_change_in_answer_count, tidy=TRUE, message=FALSE, warning=FALSE, results='hide', echo=FALSE}
...
```{r future_study_sentiment, qq3_stacked_bars_change_in_answer_count, tidy=TRUE, message=FALSE, warning=FALSE, results='hide', echo=FALSE}
