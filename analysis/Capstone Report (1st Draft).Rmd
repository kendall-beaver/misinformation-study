...
"1" = "Don't take time to fact check and verify information.",
"2" = "Believe first thing they see; source seems real, important, or is popular.",
"3" = "Look for ideas confirming pre-existing beliefs, or for entertaining, dramatic ideas.",
"4" = "Unsure.",
"5" = "Other.",
"6" = "There's too much misinformation available that gets spread around easily."
...
# Step 1: Reshape, exclude Gender, relabel columns
type_of_lie_long <- lying %>%
  select(QQ3_BEFORE, QQ3_AFTER) %>%
  pivot_longer(cols = c(QQ3_BEFORE, QQ3_AFTER),
               names_to = "QuestionType",
               values_to = "Response") %>%
  drop_na() %>%
  mutate(QuestionType = recode(QuestionType,
                               QQ3_BEFORE = "Before Ad",
                               QQ3_AFTER = "After Ad"))

# 🚫 No str_split, no unnest, no str_trim 🚫
