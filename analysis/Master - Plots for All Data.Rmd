...
plot <- ggplot(lie_counts_filtered, aes(x = QuestionType, y = perc_all, fill = Response)) +
  ...
  scale_fill_manual(
    ...
    labels = c(
      ...
      "6" = "Curious about checking out the group;\ngroup seems large, powerful & different.",
      "9" = "Other; too broad, unclear, or doesn't\nmatch any other categories."
    )
  ) +
...
# All - Summary Table (25% Threshold)

```{r summary_table_all_25, tidy=TRUE, message=FALSE, warning=FALSE, results='hide', echo=FALSE}

# Load necessary libraries
library(dplyr)
library(formattable)

# Step 1: Create change buckets for QQ1, QQ2, and QQ3 with < -25% and >= 25%
lying <- lying %>%
  mutate(
    change_bucket_QQ1 = case_when(
...
# Step 2: Create summary table for QQ1, QQ2, and QQ3 with only < -25% and >= 25%
summary_table_QQ1 <- lying %>%
  group_by(GROUP, change_bucket_QQ1) %>%
  summarise(count = n(), .groups = "drop") %>%
  group_by(GROUP) %>%
  mutate(percentage = round(100 * count / sum(count), 1)) %>%
  filter(change_bucket_QQ1 %in% c("< -25%", ">= 25%")) %>%
  select(GROUP, change_bucket_QQ1, percentage) %>%
  pivot_wider(names_from = change_bucket_QQ1, values_from = percentage, values_fill = 0)
...
summary_table_QQ2 <- lying %>%
  ...
  group_by(GROUP) %>%
  mutate(percentage = round(100 * count / sum(count), 1)) %>%
  filter(change_bucket_QQ2 %in% c("< -25%", ">= 25%")) %>%
  select(GROUP, change_bucket_QQ2, percentage) %>%
  pivot_wider(names_from = change_bucket_QQ2, values_from = percentage, values_fill = 0)
...
summary_table_QQ3 <- lying %>%
  ...
  group_by(GROUP) %>%
  mutate(percentage = round(100 * count / sum(count), 1)) %>%
  filter(change_bucket_QQ3 %in% c("< -25%", ">= 25%")) %>%
  select(GROUP, change_bucket_QQ3, percentage) %>%
  pivot_wider(names_from = change_bucket_QQ3, values_from = percentage, values_fill = 0)

# Step 3: Combine the summary tables by joining them on GROUP
final_combined <- summary_table_QQ1 %>%
  full_join(summary_table_QQ2, by = "GROUP") %>%
  full_join(summary_table_QQ3, by = "GROUP")

# Step 4: Custom highlighter function for highlighting
highlight_top_2_red <- function(column) {
  formatter("span",
            style = function(x) {
              ranks <- rank(-x, ties.method = "first")
...
# Step 5: Apply red highlighting for < -25% and green highlighting for >= 25%
formattable(final_combined, list(
  "< -25%.x" = highlight_top_2_red(final_combined$`< -25%.x`),
  "< -25%.y" = highlight_top_2_red(final_combined$`< -25%.y`),
  "< -25%"   = highlight_top_2_red(final_combined$`< -25%`),
  
  ">= 25%.x" = highlight_top_2_green(final_combined$`>= 25%.x`),
  ">= 25%.y" = highlight_top_2_green(final_combined$`>= 25%.y`),
  ">= 25%"   = highlight_top_2_green(final_combined$`>= 25%`)
))
...
# Step 1: Create change buckets for QQ1, QQ2, and QQ3 with < -50% and >= 50%
lying <- lying %>%
  mutate(
    change_bucket_QQ1 = case_when(
      composite_diff_norm_QQ1 < -0.5 ~ "< -50%",
...
# Step 2: Create summary table for QQ1, QQ2, and QQ3 with only < -50% and >= 50%
summary_table_QQ1 <- lying %>%
  group_by(GROUP, change_bucket_QQ1) %>%
  summarise(count = n(), .groups = "drop") %>%
  group_by(GROUP) %>%
  mutate(percentage = round(100 * count / sum(count), 1)) %>%
  filter(change_bucket_QQ1 %in% c("< -50%", ">= 50%")) %>%
  select(GROUP, change_bucket_QQ1, percentage) %>%
  pivot_wider(names_from = change_bucket_QQ1, values_from = percentage, values_fill = 0)
...
summary_table_QQ2 <- lying %>%
  ...
  group_by(GROUP) %>%
  mutate(percentage = round(100 * count / sum(count), 1)) %>%
  filter(change_bucket_QQ2 %in% c("< -50%", ">= 50%")) %>%
  select(GROUP, change_bucket_QQ2, percentage) %>%
  pivot_wider(names_from = change_bucket_QQ2, values_from = percentage, values_fill = 0)
...
summary_table_QQ3 <- lying %>%
  ...
  group_by(GROUP) %>%
  mutate(percentage = round(100 * count / sum(count), 1)) %>%
  filter(change_bucket_QQ3 %in% c("< -50%", ">= 50%")) %>%
  select(GROUP, change_bucket_QQ3, percentage) %>%
  pivot_wider(names_from = change_bucket_QQ3, values_from = percentage, values_fill = 0)
...
# Step 5: Apply red highlighting for < -50% and green highlighting for >= 50%
formattable(final_combined, list(
  "< -50%.x" = highlight_top_2_red(final_combined$`< -50%.x`),
  "< -50%.y" = highlight_top_2_red(final_combined$`< -50%.y`),
  "< -50%"   = highlight_top_2_red(final_combined$`< -50%`),
  
  ">= 50%.x" = highlight_top_2_green(final_combined$`>= 50%.x`),
  ">= 50%.y" = highlight_top_2_green(final_combined$`>= 50%.y`),
