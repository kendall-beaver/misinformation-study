...
lying_summary <- lying %>%
  group_by(GROUP) %>%
  summarise(
    Mean = mean(composite_diff_norm, na.rm = TRUE),
    Median = median(composite_diff_norm, na.rm = TRUE),
    SD = sd(composite_diff_norm, na.rm = TRUE),
    N = n()
  )

print(lying_summary)

# Create a summary table by group and change bucket
summary_table <- lying %>%
  group_by(GROUP, change_bucket) %>%
  summarise(count = n(), .groups = "drop") %>%
  group_by(GROUP) %>%
  mutate(
    percentage = round(100 * count / sum(count), 1)
  ) %>%
  select(GROUP, change_bucket, percentage) %>%
  pivot_wider(
    names_from = change_bucket,
